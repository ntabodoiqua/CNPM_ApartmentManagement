...
if (isset($_POST['update_resident'])) {
    ...
    // Tính toán số người đang sống nếu thay đổi trạng thái hoặc căn hộ
    if ($result_update) {
        $cal_num_living = "
        SELECT COUNT(*) as curr_living 
        FROM `residents` 
        WHERE apartment_id = '$resident_apartment' 
        AND (resident_status = 'Tạm trú' OR resident_status = 'Đang sống')
        ";
        ...
        if ($resident_relation_owner == "Chủ hộ") {
            ...
            WHERE `apartment_id` = '$resident_apartment'
            ";
            mysqli_query($con, $update_apartment_owner);
        }

        // Xử lý nếu trạng thái là đã chuyển đi
        if ($resident_status == "Đã chuyển đi") {
            $update_resident_ngaydi = "
            UPDATE `residents` 
            SET `resident_ngaydi` = NOW()  
            WHERE `resident_id` = '$resident_id'
            ";
            mysqli_query($con, $update_resident_ngaydi);
        }
        ...
        <tbody>
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <tr>
                    ...
                    <td>
                        <select id="residentStatus" name="resident_status" class="form-select" required>
                            <?php if ($resident['resident_status'] == "Tạm trú") echo "<option value='Tạm trú'>Tạm trú</option>"?>
                            <?php if ($resident['resident_status'] == "Tạm vắng") echo "<option value='Tạm vắng'>Tạm vắng</option>"?>
